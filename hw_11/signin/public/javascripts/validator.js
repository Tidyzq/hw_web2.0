var checkCases = {
    ...
    rpwd: {
        'passwords should be the same' : function(text) {
            return text == checkCases.pwdRecord;
        }
    },
    tel: {
        'should only contain numbers': function(text) {
    ...
    email: {
        ...
        'should have a valid server postfix': function(text) {
...
var finalCheck = {
    ...
    pwd : function(text) { finalCheck.pwdRecord = text; return /^[a-z0-9\-_]{6,11}$/i.test(text); },
    rpwd : function(text) { return text == finalCheck.pwdRecord;},
    id: function(text) { return /^[1-9]\d{7}$/.test(text); },
